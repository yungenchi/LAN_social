...
// Include models.
db.admin_user = require("./models/admin-user.js")(db.sequelize, DataTypes);
...
db.deprecate_post = require("./models/deprecate-post.js")(db.sequelize, DataTypes);
...
// db.stats_login.belongsTo(db.user, {foreignKey: {name: "username", allowNull: false}});

// Include a sync option with seed data logic included.
db.sync = async () => {
  // Sync schema.
  await db.sequelize.sync();
  // Import seed data if necessary
  await seedData();
};

async function seedData() {
  const admin_userCount = await db.admin_user.count();
  ...
  if(admin_userCount === 0){
    ...
    await db.admin_user.create({ username: "ian000", password_hash: hash, firstname: "ian", lastname : "chi" });

    hash = await argon2.hash("0000", { type: argon2.argon2id });
    await db.admin_user.create({ username: "zico000", password_hash: hash, firstname: "zico", lastname : "zhong" });

    hash = await argon2.hash("0000", { type: argon2.argon2id });
    await db.admin_user.create({ username: "zupei000", password_hash: hash, firstname: "zupei", lastname : "min" });
  }
